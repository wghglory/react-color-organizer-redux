...
import AddColorForm from '../../../app/components/ui/AddColorForm';
...
describe("<AddColorForm /> UI Componnet", () => {
  ...
  describe("Rendering the UI", () => {
    ...
    it("Submitting default onNewColor does not cause error", () => {
      ...
      wrapper.find('input').get(0).value = 'test-color';
      wrapper.find('input').get(1).value = '#0000FF';
  ...
  describe("Adding New Colors", () => {
    ...
    beforeAll(() => {
      ...
      wrapper.find('input').get(0).value = 'test-color';
      wrapper.find('input').get(1).value = '#000099';
      wrapper.find('form').simulate('submit');
    });
    ...
    it("resets the color value", () =>
      ...
      expect(wrapper.find('input').get(1).value).toBe("#000000")
    );
